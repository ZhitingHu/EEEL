...
Path* Hierarchy::FindPathBetweenEntities(int entity_from, int entity_to) {
  Path* path = new Path();
  vector<int> sorted_common_ancestors;
...
void Hierarchy::FindCommonAncestors(const int entity_from, const int entity_to,
  ...
  while (!unprocessed_nodes.empty()) {
    ...
    ++entity_from_ancestor_weights[nodes_[cur_node_idx]->id()];
  ...
  while (!unprocessed_nodes.empty()) {
    cur_node_idx = unprocessed_nodes.pop();
    ++entity_to_ancestor_weights[nodes_[cur_node_idx]->id()];
    const vector<int> cur_parents = cur_node->parent_idx();
    for (int p_idx_i = 0; p_idx_i < cur_parents.size(); ++p_idx_i) {
      ...
      // entity_from itself)
      if (entity_from_ancestor_idx.find(cur_parent_idx) 
          != entity_from_ancestor_idx.end() &&
